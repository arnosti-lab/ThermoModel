...
{ 
  cmaes_WriteToFileAW(t, key, name, (char *)"a"); /* default is append */
}
...
{
  const char *s  = (const char *) "signals.par"; 
  FILE *fp;
  if (filename == NULL)
    filename = s;
  fp = fopen( filename, "r"); 
...
*/
{
  ...
  keys[0]  = (char *) " stop%98s %98s";        /* s=="now" or eg "MaxIter+" %lg"-number */
                                     /* works with and without space */
  keys[1]  = (char *) " print %98s %98s";       /* s==keyword for WriteFile */
  keys[2]  = (char *) " write %98s %128s %98s"; /* s1==keyword, s2==filename */
  keys[3]  = (char *) " check%98s %98s";
  keys[4]  = (char *) " maxTimeFractionForEigendecompostion %98s";
  ckeys = 5; 
          ...
          if((nb=sscanf(s, keys[ikey], sin1, sin2, sin3, sin4)) >= 1) 
            {
              ...
              case 2 : /* "write" */
                ...
                if (t->countevals < t->sp.lambda && t->flgresumedone == 0) 
                  cmaes_WriteToFileAW(t, sin1, sin2, (char*)"w"); /* overwrite */
               ...
               const char * filename)
{
  ...
  /* All scalars:  */
  i = 0;
  t->rgsformat[i]  = (char *) " N %d";        t->rgpadr[i++] = (void *) &t->N; 
  t->rgsformat[i]  = (char *) " seed %d";    t->rgpadr[i++] = (void *) &t->seed;
  t->rgsformat[i]  = (char *) " stopMaxFunEvals %lg"; t->rgpadr[i++] = (void *) &t->stopMaxFunEvals;
  t->rgsformat[i]  = (char *) " stopMaxIter %lg"; t->rgpadr[i++] = (void *) &t->stopMaxIter;
  t->rgsformat[i]  = (char *) " stopFitness %lg"; t->rgpadr[i++]=(void *) &t->stStopFitness.val;
  t->rgsformat[i]  = (char *) " stopTolFun %lg"; t->rgpadr[i++]=(void *) &t->stopTolFun;
  t->rgsformat[i]  = (char *) " stopTolFunHist %lg"; t->rgpadr[i++]=(void *) &t->stopTolFunHist;
  t->rgsformat[i]  = (char *) " stopTolX %lg"; t->rgpadr[i++]=(void *) &t->stopTolX;
  t->rgsformat[i]  = (char *) " stopTolUpXFactor %lg"; t->rgpadr[i++]=(void *) &t->stopTolUpXFactor;
  t->rgsformat[i]  = (char *) " lambda %d";      t->rgpadr[i++] = (void *) &t->lambda;
  t->rgsformat[i]  = (char *) " mu %d";          t->rgpadr[i++] = (void *) &t->mu;
  t->rgsformat[i]  = (char *) " weights %5s";    t->rgpadr[i++] = (void *) t->weigkey;
  t->rgsformat[i]  = (char *) " fac*cs %lg";t->rgpadr[i++] = (void *) &t->cs;
  t->rgsformat[i]  = (char *) " fac*damps %lg";   t->rgpadr[i++] = (void *) &t->damps;
  t->rgsformat[i]  = (char *) " ccumcov %lg";    t->rgpadr[i++] = (void *) &t->ccumcov;
  t->rgsformat[i]  = (char *) " mucov %lg";     t->rgpadr[i++] = (void *) &t->mucov;
  t->rgsformat[i]  = (char *) " fac*ccov %lg";  t->rgpadr[i++]=(void *) &t->ccov;
  t->rgsformat[i]  = (char *) " diagonalCovarianceMatrix %lg"; t->rgpadr[i++]=(void *) &t->diagonalCov;
  t->rgsformat[i]  = (char *) " updatecov %lg"; t->rgpadr[i++]=(void *) &t->updateCmode.modulo;
  t->rgsformat[i]  = (char *) " maxTimeFractionForEigendecompostion %lg"; t->rgpadr[i++]=(void *) &t->updateCmode.maxtime;
  t->rgsformat[i]  = (char *) " resume %59s";    t->rgpadr[i++] = (void *) t->resumefile;
  t->rgsformat[i]  = (char *) " fac*maxFunEvals %lg";   t->rgpadr[i++] = (void *) &t->facmaxeval;
  t->rgsformat[i]  = (char *) " fac*updatecov %lg"; t->rgpadr[i++]=(void *) &t->facupdateCmode;
  t->n1para = i; 
  t->n1outpara = i-2; /* disregard last parameters in WriteToFile() */

  /* arrays */
  i = 0;
  t->rgskeyar[i]   = (char *) " typicalX %d";   t->rgp2adr[i++] = &t->typicalX;
  t->rgskeyar[i]   = (char *) " initialX %d";   t->rgp2adr[i++] = &t->xstart;
  t->rgskeyar[i]   = (char *) " initialStandardDeviations %d"; t->rgp2adr[i++] = &t->rgInitialStds;
  t->rgskeyar[i]   = (char *) " diffMinChange %d"; t->rgp2adr[i++] = &t->rgDiffMinChange;
  t->n2para = i;  
...
{
  char s[1000], *ss  = (char *) "initials.par";
  int ipara, i;
  int size;
  FILE *fp;
  if (filename == NULL)
